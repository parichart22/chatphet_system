...
<table class="table">
  ...
  <tbody>
<?php
    $result = mysqli_query($con,"SELECT sum(amount),pk_product FROM order_detail WHERE DATE(datetime) BETWEEN '".$_REQUEST['datestart']."' and '".$_REQUEST['dateend']."' and comid='".$_REQUEST['company']."' GROUP by pk_product ORDER by sum(amount) DESC");
